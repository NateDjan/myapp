...
const quizBanks = JSON.parse(
  fs.readFileSync(path.join(__dirname, '..', 'content', 'quizBanks.fr.json'), 'utf8')
);
...
function generateQuizFromBank(subject, randomInt) {
  ...
  if (!bank || bank.length === 0) {
    return {
      type: 'quiz',
      prompt: `Revision ${subject} : donne une notion importante vue en cours.`,
      expected: 'notion',
