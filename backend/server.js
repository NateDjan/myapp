...
function auth(req, res, next) {
  ...
  const row = db.prepare('SELECT parent_id FROM auth_tokens WHERE token = ?').get(token);
...
app.post('/api/parents/register', (req, res) => {
  ...
  try {
    const insert = db.prepare('INSERT INTO parents (name, email, password, created_at) VALUES (?, ?, ?, ?)');
    const result = insert.run(name, email.toLowerCase(), hashed, nowIso());
...
app.post('/api/parents/login', (req, res) => {
  ...
  const hashed = crypto.createHash('sha256').update(parsed.data.password).digest('hex');
  const parent = db.prepare('SELECT id, name FROM parents WHERE email = ? AND password = ?').get(parsed.data.email.toLowerCase(), hashed);
  if (!parent) return res.status(401).json({ error: 'Invalid credentials' });

  const token = crypto.randomUUID();
  db.prepare('INSERT INTO auth_tokens (token, parent_id, created_at) VALUES (?, ?, ?)').run(token, parent.id, nowIso());
  return res.json({ token, parent });
});
...
app.post('/api/parents/children', auth, (req, res) => {
  ...
  const result = db.prepare(`
    INSERT INTO children (parent_id, first_name, grade, age, strengths, weaknesses, created_at)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(
    req.parentId,
...
app.post('/api/evaluation/:childId', auth, (req, res) => {
  ...
  db.prepare('INSERT INTO activity_log (child_id, activity_type, score, points_delta, payload, created_at) VALUES (?, ?, ?, ?, ?, ?)')
    .run(childId, 'evaluation', score, 0, JSON.stringify({ readingLevel, spellingLevel }), nowIso());
...
app.get('/api/lesson/:childId', auth, (req, res) => {
  ...
  const reading = db.prepare('SELECT * FROM phrase_bank WHERE subject = ? AND mode = ? AND level = ? LIMIT 1').get(subject, subject === 'Francais' ? 'lecture' : 'quiz', level) ||
    db.prepare('SELECT * FROM phrase_bank WHERE subject = ? LIMIT 1').get(subject);
  const dictation = subject === 'Francais'
  ...
  const review = db.prepare(`
    ...
    WHERE child_id = ? AND status = 'pending' AND datetime(next_review_at) <= datetime('now')
    ORDER BY next_review_at ASC
    LIMIT 1
  `).get(childId);

  res.json({ lesson: reading, dictation, review });
});
...
app.post('/api/session/:childId/dictation', auth, (req, res) => {
  ...
  db.prepare('INSERT INTO activity_log (child_id, activity_type, score, points_delta, payload, created_at) VALUES (?, ?, ?, ?, ?, ?)')
    .run(childId, 'dictation', score, points, JSON.stringify({ expected: parsed.data.expected }), nowIso());

  if (score < 100) {
    db.prepare('INSERT INTO review_queue (child_id, phrase, interval_days, next_review_at, status) VALUES (?, ?, ?, ?, ?)')
      .run(childId, parsed.data.expected, 1, addDays(1), 'pending');
  }
...
app.post('/api/review/:reviewId/complete', auth, (req, res) => {
  ...
  const review = db.prepare(`
    SELECT r.*, c.parent_id FROM review_queue r
...
app.post('/api/homework/:childId', auth, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO homework (child_id, subject, title, details, due_date, source) VALUES (?, ?, ?, ?, ?, ?)')
